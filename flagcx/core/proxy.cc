...
FLAGCX_TEMPLETELIST_DEFINE(ProdProgChannel, struct flagcxProxyOps, prodPrevChannel, prodNextChannel);
...
static flagcxResult_t asyncProxyOpDequeue(flagcxProxyAsyncOp **opHead, flagcxProxyAsyncOp *op){
  if(*opHead == op) *opHead = op->next;
  if(op->next) op->next->prev = op->prev;
  if(op->prev) op->prev->next = op->next;
  if(op->reqSize) free(op->reqBuff);
  if(op->respSize) free(op->respBuff);
  free(op);
  return flagcxSuccess;
}
...
static flagcxResult_t SaveProxy(struct flagcxHeteroComm* comm, struct flagcxChannel* channel, int type, int peer, struct flagcxProxyOp* op, int connIndex, bool* justInquire) {
  ...
  if (justInquire) *justInquire = true;
    ...
    queue = type == proxySend ? &proxyOps->prodPeers.sendQueue : &proxyOps->prodPeers.recvQueue;

    pthread_mutex_lock(&comm->proxyState->mutex);
    flagcxProdProgChannelListEnList(&comm->proxyState->prodProgChannelHead, proxyOps);
...
flagcxResult_t flagcxProxySaveOp(struct flagcxHeteroComm* comm, struct flagcxProxyOp* op, bool* justInquire) {
  ...
  case flagcxPatternRecv: {
      if (op->root == comm->rank) return flagcxSuccess;
      FLAGCXCHECK(SaveProxy(comm, channel, op->pattern == flagcxPatternSend ? proxySend : proxyRecv, op->root, op, 0, justInquire));
    } break;
  }
  return flagcxSuccess;
}
...
static void flagcxProgressQueEmptyCheck(struct flagcxProxyState* proxyState){
  ...
  for(int i=0;i<MAXCHANNELS;i++){
    if(!flagcxProgPeerListEmpty(proxyState->proxyOps[i].consProgPeerHead)) error = 1;
    for(int r=0;r<proxyState->nRanks;r++){
      if(!flagcxIntruQueueEmpty(&proxyState->proxyOps[i].consPeers[r].sendQueue) ||
         !flagcxIntruQueueEmpty(&proxyState->proxyOps[i].consPeers[r].recvQueue))
       error = 1;
    }
  ...
  if(error) INFO(FLAGCX_INIT, "progress queue is not empty");
}
  ...
  while(!stop || !commplete){
    ...
    if(!flagcxConsProgChannelListEmpty(proxyState->consProgChannelHead)){
      ...
      do{
        ...
        if(!flagcxProgPeerListEmpty(proxyOps->consProgPeerHead)){
          ...
          do{
            ...
            if(!flagcxIntruQueueEmpty(queue)){
              commplete = false;
              struct flagcxProxyOp* op = flagcxIntruQueueHead(queue);
              struct recvNetResources *resources = (recvNetResources *)op->connection->transportResources;
            ...
            if(flagcxIntruQueueEmpty(&peer->sendQueue) && flagcxIntruQueueEmpty(&peer->recvQueue)){
        ...
        if(flagcxProgPeerListEmpty(proxyOps->consProgPeerHead)){
          flagcxConsProgChannelListDeLete(&proxyState->consProgChannelHead, proxyOps);
        }
        proxyOps = next;
      }while(proxyOps != NULL);
    }
    ...
    while(!flagcxProdProgChannelListEmpty(proxyState->prodProgChannelHead)){
      ...
      while(!flagcxIntruQueueEmpty(queue)){
        ...
        flagcxProgPeerListEnList(&proxyOps->consProgPeerHead, &proxyOps->consPeers[op->root]);
...
static flagcxResult_t expectedProxyResponseStore(struct flagcxProxyState* state, void* opId, void* respBuff, int respSize, flagcxResult_t res) {
...
static flagcxResult_t expectedProxyResponseEnqueue(struct flagcxProxyState* state, void* opId, int respSize) {
...
static flagcxResult_t expectedProxyResponseRemove(struct flagcxProxyState* state, void* opId) {
  struct flagcxExpectedProxyResponse* elem = state->expectedResponses;
  struct flagcxExpectedProxyResponse* prev = NULL;
  while (elem) {
    if (elem->opId == opId) {
...
flagcxResult_t flagcxPollProxyResponse(struct flagcxHeteroComm* comm, struct flagcxProxyConnector* proxyConn, void* respBuff, void* opId) {
  ...
  int found = 0;
  flagcxResult_t res = expectedProxyResponseDequeue(sharedProxyState, opId, respBuff, &found);
  ...
  if (found == 0) {
    ...
    int offset = 0;
    if (flagcxSuccess != flagcxSocketProgress(FLAGCX_SOCKET_RECV, sock, &resp, sizeof(resp), &offset)) {
    ...
    // If we've returned a partial response, block to receive the rest of it
      ...
      while (offset < sizeof(resp))
        FLAGCXCHECK(flagcxSocketProgress(FLAGCX_SOCKET_RECV, sock, &resp, sizeof(resp), &offset));
    }

    INFO(FLAGCX_PROXY, "flagcxPollProxyResponse Received new opId=%p", resp.opId);
    ...
    if (resp.opId == opId) {
      INFO(FLAGCX_PROXY, "resp.opId=%p matches expected opId=%p", resp.opId, opId);
      FLAGCXCHECK(expectedProxyResponseRemove(sharedProxyState, resp.opId));
      return resp.res;
    } else {
      INFO(FLAGCX_PROXY, "Queuing opId=%p respBuff=%p respSize=%d", resp.opId, respBuff, resp.respSize);
      // Store the result and mark response as completed
      FLAGCXCHECK(expectedProxyResponseStore(sharedProxyState, resp.opId, respBuff, resp.respSize, resp.res));
      return flagcxInProgress;
    }
  } else {
...
static flagcxResult_t proxyProgressAsync(flagcxProxyAsyncOp **opHead, flagcxProxyAsyncOp *op) {
  ...
  if(op->type == flagcxProxyMsgConnect){
    if(op->connection->send){
      struct sendNetResources *resources = (struct sendNetResources *) op->connection->transportResources;
      if(!resources->netSendComm){
        FLAGCXCHECK(flagcxNetIb.connect(resources->netDev, (void *) op->reqBuff, &resources->netSendComm, NULL));
      }
      else{
        FLAGCXCHECK(flagcxNetIb.regMr(resources->netSendComm, resources->buffers[0], resources->buffSizes[0], 2, &resources->mhandles[0]));
        done = 1;
      }
    }else{
      struct recvNetResources *resources = (struct recvNetResources *) op->connection->transportResources;
      if(!resources->netRecvComm){
        FLAGCXCHECK(flagcxNetIb.accept(resources->netListenComm, &resources->netRecvComm, NULL));
      }
      else{
        FLAGCXCHECK(flagcxNetIb.regMr(resources->netRecvComm, resources->buffers[0], resources->buffSizes[0], 2, &resources->mhandles[0]));
        done = 1;
      } 
    }
  }
  else return flagcxInternalError;

  if (done) {
    INFO(FLAGCX_PROXY, "proxyProgressAsync opId=%p op.type=%d op.reqBuff=%p op.respSize=%d done", op->opId, op->type, op->reqBuff, op->respSize);
     ...
     * flagcxSocketSend might already send the respBuff to the requester. If we still choose
    ...
    if (op->respSize) {
      // Send the response
      FLAGCXCHECK(flagcxSocketSend(op->connection->sock, op->respBuff, op->respSize));
    }
...
flagcxResult_t flagcxProxyCallAsync(struct flagcxHeteroComm* comm, struct flagcxProxyConnector* proxyConn, int type, void* reqBuff, int reqSize, int respSize, void* opId) {
  ...
  if (sock == NULL) return flagcxInternalError;

  FLAGCXCHECKGOTO(flagcxSocketSend(sock, &type, sizeof(int)), ret, error);
  FLAGCXCHECKGOTO(flagcxSocketSend(sock, &proxyConn->connection, sizeof(void*)), ret, error);
  ...
  if (reqSize) FLAGCXCHECKGOTO(flagcxSocketSend(sock, reqBuff, reqSize), ret, error);
...
static flagcxResult_t proxyServiceInitOp(int type, struct flagcxSocket *sock, struct flagcxProxyAsyncOp **opHead, flagcxHeteroComm_t comm) {
  ...
  if (asyncOp->respSize) FLAGCXCHECK(flagcxCalloc(&asyncOp->respBuff, asyncOp->respSize));
...
flagcxResult_t flagcxProxyCallBlocking(struct flagcxHeteroComm* comm, struct flagcxProxyConnector* proxyConn, int type, void* reqBuff, int reqSize, void* respBuff, int respSize) {
  ...
  FLAGCXCHECKGOTO(flagcxProxyCallAsync(comm, proxyConn, type, reqBuff, reqSize, respSize, opId), res, fail);
...
flagcxResult_t flagcxProxyInit(struct flagcxHeteroComm* comm) {
  ...
  FLAGCXCHECK(flagcxSocketInit(&comm->proxyState->ipcSock, &bootstrapNetIfAddr, comm->magic, flagcxSocketTypeProxy, NULL, 1));
  FLAGCXCHECK(flagcxSocketListen(&comm->proxyState->ipcSock));

  flagcxSocket *proxySock = &comm->proxyState->peerSock;
  FLAGCXCHECK(flagcxSocketInit(proxySock, &comm->proxyState->ipcSock.addr, comm->magic, flagcxSocketTypeProxy));
  ...
  pthread_create(&comm->proxyState->thread, NULL, flagcxProxyService, (void *)comm);
...
void *flagcxProxyService(void* args){
  ...
  FLAGCXCHECKGOTO(flagcxSocketAccept(&sock, &comm->proxyState->ipcSock), res, out);
  ...
  while(!stop || (stop && opHead)){
    int type, closed;
    res = flagcxSocketTryRecv(&sock, &type, sizeof(int), &closed, false /*blocking*/);
    if (res != flagcxSuccess && res != flagcxInProgress) {
      WARN("[Service thread] Could not receive type from localRank %d, res=%u, closed=%d", comm->rank, res, closed);
    } else if (closed) {
      INFO(FLAGCX_NET, "[Service thread] Connection closed by localRank %d", comm->rank);
  ...
  for(int peer=0;peer<comm->nRanks;peer++){
    for(int c=0;c<MAXCHANNELS;c++){
      if(comm->channels[c].peers[peer]->recv[0].connected == 1){
        ...
        struct recvNetResources* resources = (struct recvNetResources*)conn->proxyConn.connection->transportResources;
      ...
      if(comm->channels[c].peers[peer]->send[0].connected == 1){
        ...
        struct sendNetResources* resources = (struct sendNetResources*)conn->proxyConn.connection->transportResources;
