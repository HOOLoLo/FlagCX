...
flagcxResult_t cnclAdaptorGetUniqueId(flagcxUniqueId_t *uniqueId) {
    ...
    return (flagcxResult_t)c2f_ret_map[cnclGetCliqueId((cnclCliqueId *)(*uniqueId))];
}
...
flagcxResult_t cnclAdaptorGather(const void* sendbuff, void* recvbuff, size_t count,
    ...
    res = cnclSend(const_cast<void*>(sendbuff), size, cnclChar, root, comm->base, stream->base);
    res = cnclGroupEnd();
...
flagcxResult_t cnclAdaptorScatter(const void* sendbuff, void* recvbuff, size_t count,
    ...
    if (rank == root) {
        for (int r = 0; r < nranks; r++) {
            res = cnclSend(const_cast<void*>(static_cast<const void*>(buffer + r * size)),
...
flagcxResult_t cnclAdaptorAlltoAll(const void* sendbuff, void* recvbuff, size_t count,
    ...
    res = cnclGroupStart();
    for (int r = 0; r < nranks; r++) {
        res = cnclSend(const_cast<void*>(static_cast<const void*>(buffer_in + r * size)),
                       size, cnclChar, r, comm->base, stream->base);
        res = cnclRecv(static_cast<void*>(buffer_out + r * size), size, cnclChar, r,
                       comm->base, stream->base);
    }
    res = cnclGroupEnd();
...
struct flagcxCCLAdaptor cnclAdaptor = {
  "CNCL",
  // Basic functions
  cnclAdaptorGetVersion,
  cnclAdaptorGetUniqueId,
  cnclAdaptorGetErrorString,
  cnclAdaptorGetLastError,
  // Communicator functions
  cnclAdaptorCommInitRank,
  cnclAdaptorCommFinalize,
  cnclAdaptorCommDestroy,
  cnclAdaptorCommAbort,
  cnclAdaptorCommResume,
  cnclAdaptorCommSuspend,
  cnclAdaptorCommCount,
  cnclAdaptorCommCuDevice,
  cnclAdaptorCommUserRank,
  cnclAdaptorCommGetAsyncError,
  // Communication functions
  cnclAdaptorReduce,
  cnclAdaptorGather,
  cnclAdaptorScatter,
  cnclAdaptorBroadcast,
  cnclAdaptorAllReduce,
  cnclAdaptorReduceScatter,
  cnclAdaptorAllGather,
  cnclAdaptorAlltoAll,
  cnclAdaptorSend,
  cnclAdaptorRecv,
  // Group semantics
